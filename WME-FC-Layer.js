...
const _stateSettings = {
    global: {
        ...
        getFeatureRoadType(feature, layer) {
            return this.getRoadTypeFromFC(feature.attributes[layer.fcPropName], layer);
        },
        getRoadTypeFromFC(fc, layer) {
            return Object.keys(layer.roadTypeMap).find(roadType => layer.roadTypeMap[roadType].indexOf(fc) !== -1);
        },
        isPermitted(stateAbbr) {
            if (BETA_IDS.indexOf(_uid) !== -1) {
                return true;
            }
            const state = _stateSettings[stateAbbr];
        ...
        getMapLayer(stateAbbr, layerID) {
            return _stateSettings[stateAbbr].fcMapLayers.find(layer => layer.layerID === layerID);
        }
    },
    AL: {
        ...
        information: {
            ...
            Description: 'Federal and State highways set to a minimum of mH.'
        },
        getWhereClause(context) {
            return context.mapContext.zoom < 4 ? `${context.layer.fcPropName}<>7` : null;
        },
        getFeatureRoadType(feature, layer) {
    ...
    AK: {
        ...
        information: {
            ...
            Permission: 'Visible to R3+',
            Description: 'Raw unmodified FC data.'
        },
        getWhereClause(context) {
            return context.mapContext.zoom < 4 ? `${context.layer.fcPropName}<>7` : null;
        },
        getFeatureRoadType(feature, layer) {
            return layer.getFeatureRoadType ? layer.getFeatureRoadType(feature) : _stateSettings.global.getFeatureRoadType(feature, layer);
        }
    },
    AZ: {
        ...
        fcMapLayers: [{
            ...
            roadTypeMap: {
                Fw: [1, 11], Ew: [2, 3, 12], MH: [4, 14], mH: [6, 16], PS: [7, 17, 8, 18], St: []
            },
            maxRecordCount: 1000,
            supportsPagination: false
        }],
        ...
        getFeatureRoadType(feature, layer) {
            const roadID = feature.attributes.RouteId.trim().replace(/  +/g, ' ');
        ...
        getFeatureRoadType(feature, layer) {
            ...
            if (layer.layerID === 7) {
                ...
                if ((fips === 19 && route === 'COLORADO BD')
                    || (fips === 37 && (route === 'GRAND AV' || route === 'S H6'))) {
                    fc = 3;
    ...
    DE: {
        ...
        information: {
            Source: 'Delaware FirstMap',
            Permission: 'Visible to R3+',
            Description: 'Raw unmodified FC data.'
        },
        getWhereClause(context) {
            return context.mapContext.zoom < 4 ? `${context.layer.fcPropName} <> 'Local'` : null;
        },
        getFeatureRoadType(feature, layer) {
            return layer.getFeatureRoadType ? layer.getFeatureRoadType(feature) : _stateSettings.global.getFeatureRoadType(feature, layer);
        }
    ...
    IA: {
        ...
        getFeatureRoadType(feature, layer) {
            const attr = feature.attributes;
            let fc = parseInt(attr[layer.fcPropName]);
    ...
    KS: {
        ...
        getFeatureRoadType(feature, layer) {
            ...
            const isFw = parseInt(attr.ACCESS_CONTROL) === 1;
    ...
    MD: {
        ...
        getFeatureRoadType(feature, layer) {
            const attr = feature.attributes;
            let fc = parseInt(attr.FUNCTIONAL_CLASS);
    ...
    NH: {
        ...
        getFeatureRoadType(feature, layer) {
            let fc = parseInt(feature.attributes[layer.fcPropName]);
            if (!(fc > 0)) { fc = 7; }
    ...
    NC: {
        ...
        getWhereClause(context) {
            if (context.mapContext.zoom < 4) {
                const clause = `(${context.layer.fcPropName} < 7 OR RTE_1_CLSS_CD IN ('I','FED','NC','RMP','US'))`;
                return clause;
            } else {
                return null;
            }
    ...
    OK: {
        ...
        getWhereClause(context) {
            if (context.mapContext.zoom < 4) {
                const clause = `(${context.layer.fcPropName} < 7 OR ROUTE_CLAS IN ('U','S','I'))`;
                return clause;
            } else {
                return null;
            }
        ...
        getFeatureRoadType(feature, layer) {
            let fc = feature.attributes[layer.fcPropName];
            const route = (feature.attributes.F_PRIMARY_ || '').trim();
            const isBusinessOrSpur = /BUS$|SPR$/i.test(route);
            const prefix = isBusinessOrSpur ? route.substring(0, 1) : feature.attributes.ROUTE_CLAS;
            const isFw = parseInt(feature.attributes.ACCESS_CON) === 1;
    ...
    SC: {
        ...
        getFeatureRoadType(feature, layer) {
            ...
            const roadType = parseInt(roadID.slice(3, 4));
            const isFw = roadType === 1;
            const isUS = roadType === 2;
            const isState = roadType === 4;
            const isBiz = parseInt(roadID.slice(-2, -1)) === 7;
            let fc = 7;
    ...
    SD: {
        ...
        getFeatureRoadType(feature, layer) {
            ...
            const isPaved = parseInt(attr.SURFACE_TYPE) > 5;
            if (isFw) {
                fc = 1;
            } else if (fc > 4 && isUS) {
                fc = (isBiz ? 6 : 4);
    ...
    TN: {
        ...
        information: {
            Source: 'Shelby County, Nashville Area MPO', Permission: 'Visible to R3+',
    ...
    UT: {
        ...
        getWhereClause(context) {
            let clause = `${context.layer.fcPropName}<=7`;
            if (context.mapContext.zoom < 4) {
        ...
        getFeatureRoadType(feature, layer) {
            ...
            let fc = feature.attributes.FC_CODE;
    ...
    VA: {
        ...
        getFeatureRoadType(feature, layer) {
            if (layer.getFeatureRoadType) {
                ...
                const rtNum = parseInt((layer.layerID === 1) ? feature.attributes.ROUTE_NO : (match ? match[2] : 99999));
...
function getVisibleStateAbbrs() {
    ...
    W.model.states.additionalInfo.forEach(function (state) {
        ...
        const activeStateAbbr = _settings.activeStateAbbr;
...
function getUrl(context, queryType, queryParams) {
    const extent = context.mapContext.extent,
        zoom = context.mapContext.zoom,
        layer = context.layer,
        state = context.state;

    const whereParts = [];
    const geometry = { xmin: extent.left, ymin: extent.bottom, xmax: extent.right, ymax: extent.top, spatialReference: { wkid: 102100, latestWkid: 3857 } };
    ...
    if (queryType === 'countOnly') {
        ...
        if (queryType === 'idRange') {
            const idPropName = context.layer.idPropName;
...
function convertFcToRoadTypeVectors(feature, state, stateAbbr, layer, zoom) {
    ...
    log(feature, 3);
    const zIndex = _stateSettings.global.roadTypes.indexOf(roadType) * 100;
    const vectors = [];
    const lineFeatures = [];
    const attr = {
    ...
    feature.geometry.paths.forEach(function (path) {
        ...
        const vectorFeature = new OL.Feature.Vector(new OL.Geometry.LineString(pointList), attr);
    ...
    if (!context.parentContext.cancel) {
        return getAsync(url, context).bind(context).then(function (res) {
            ...
            if (res.objectIds) {
                ...
                let currentIndex = 0;
                const offset = Math.min(this.layer.maxRecordCount, 1000);
            ...
            const context = this;
            if (!context.parentContext.cancel) {
                const url = getUrl(this, 'idRange', idRange);
                log(url, 2);
                    ...
                    if (!context.parentContext.cancel) {
                        ...
                        features.forEach(function (feature) {
                            if (!res.context.parentContext.cancel) {
                                const vector = convertFcToRoadTypeVectors(feature, context.state, context.stateAbbr, context.layer, context.mapContext.zoom);
...
function fetchStateFC(context) {
    const state = _stateSettings[context.stateAbbr];
    const contexts = [];
    state.fcMapLayers.forEach(function (layer) {
        contexts.push({ parentContext: context.parentContext, layer, state, stateAbbr: context.stateAbbr, mapContext: context.mapContext });
    });
...
function fetchAllFC() {
    ...
    const contexts = [];
    const parentContext = { callCount: 0,/*existingFcFeatureUniqueIds:{}, addedFcFeatureUniqueIds:[],*/ startTime: Date.now() };
    ...
    getVisibleStateAbbrs().forEach(function (stateAbbr) {
        contexts.push({ parentContext, stateAbbr, mapContext });
    });
    const map = Promise.map(contexts, function (context) {
...
function initLayer() {
    const _drawingContext = {
        ...
        getStrokeWidth() { return getLineWidth(); }
    };
    const defaultStyle = new OL.Style({
    ...
    _mapLayer.setZIndex(MAP_LAYER_ZINDEX);
    ...
    const checkLayerZIndex = function () {
        if (_mapLayer.getZIndex() != MAP_LAYER_ZINDEX) {
            log(`ADJUSTED FC LAYER Z-INDEX ${MAP_LAYER_ZINDEX}, ${_mapLayer.getZIndex()}`, 1);
            _mapLayer.setZIndex(MAP_LAYER_ZINDEX);
        }
    };
...
function initUserPanel() {
    ...
    //     fetchAllFC();
    // });
    for (const stateAbbr in _stateSettings) {
        if (stateAbbr !== 'global') {
...
function loadStateFCInfo() {
    ...
    if (_stateSettings[_settings.activeStateAbbr]) {
        const stateInfo = _stateSettings[_settings.activeStateAbbr].information;
        const $panelStateInfo = $('<dl>');
        for (const propertyName in stateInfo) {
...
function processText(text) {
    return new Promise(function (resolve, reject) {
        const newText = text.replace(/(e)/, 'E');
        resolve(newText);
    });
}

function init() {
    if (DEBUG_LEVEL > 0 && Promise.config) {
        Promise.config({
            warnings: true,
