...
(async function main() {
    ...
    const STATE_SETTINGS = {
        global: {
            ...
            getFeatureRoadType(feature, layer) {
                const fc = feature.properties[layer.fcPropName];
        ...
        AL: {
            ...
            getFeatureRoadType(feature, layer) {
                let fc = parseInt(feature.properties[layer.fcPropName], 10);
                if (fc > 4 && feature.properties.State_Sys === 'YES') { fc = 4; }
        ...
        CA: {
            ...
            getFeatureRoadType(feature, layer) {
                const fc = parseInt(feature.properties[layer.fcPropName], 10);
                return STATE_SETTINGS.global.getRoadTypeFromFC(fc, layer);
            }
        },
        CO: {
        ...
        CT: {
            ...
            getFeatureRoadType(feature, layer) {
                let fc = parseInt(feature.properties[layer.fcPropName], 10); //JS55CT
                if (fc > 4 && feature.properties.State_Sys === 'YES') { //JS55CT
                    fc = 4;
                }
                return STATE_SETTINGS.global.getRoadTypeFromFC(fc, layer);
            }
        ...
        LA: {
            ...
            getFeatureRoadType(feature, layer) {
                let fc = feature.properties[layer.fcPropName];
                if (fc === '2a' || fc === '2b') { fc = 2; }
                fc = parseInt(fc, 10);
                const route = feature.properties.RouteID.split('_')[1].trim();
        ...
        MT: {
            ...
            getFeatureRoadType(feature, layer) {
                ...
                const roadID = feature.properties.SIGN_ROUTE || feature.properties.ROUTE_NAME;
        ...
        NH: {
            ...
            getFeatureRoadType(feature, layer) {
                let fc = parseInt(feature.properties[layer.fcPropName], 10);
                if (!(fc > 0)) { fc = 7; }
                const route = feature.properties.STREET_ALIASES;
                const isUS = /US /.test(route);
                const isState = /NH /.test(route);
                if (fc === 2) fc = feature.properties.TIER === 1 ? 1 : 3;
        ...
        NM: {
            ...
            getFeatureRoadType(feature, layer) {
                let fc = parseInt(feature.properties[layer.fcPropName], 10);
                const roadType = feature.properties.D_RT_ROUTE.split('-', 1).shift();
        ...
        NY: { // https://gis.dot.ny.gov/hostingny/rest/services/Basemap/MapServer/21
            ...
            getFeatureRoadType(feature, layer) {
                ...
                if (layer.layerID === 'Basemap/MapServer/21') {
                    ...
                    const routeNo = feature.properties.ROUTE_NO; //JS55CT
                    if (/^NY.*/.test(routeNo)) {
        ...
        NC: {
            ...
            getHwySys(feature) {
                let hwySys;
                switch (feature.properties.RouteClass.toString()) {
                    case '1':
                        hwySys = 'interstate';
                        break;
                    case '2':
            ...
            isBusinessRoute(feature) {
                const qual = feature.properties.RouteQualifier.toString();
                return qual === '9';
            }
        },
        ND: {
        ...
        OH: {
            ...
            getFeatureRoadType(feature, layer) {
                let fc = feature.properties[layer.fcPropName];
                const prefix = feature.properties.ROUTE_TYPE;
        ...
        OK: {
            ...
            getFeatureRoadType(feature, layer) {
                let fc = feature.properties[layer.fcPropName];
                const route = (feature.properties.FHWAPRIMARYROUTE || '').trim();
                const isBusinessOrSpur = /BUS$|SPR$/i.test(route);
                const prefix = isBusinessOrSpur ? route.substring(0, 1) : feature.properties.ODOTROUTECLASS;
                const isFw = parseInt(feature.properties.ACCESSCONTROL, 10) === 1;
        ...
        RI: {
            ...
            getFeatureRoadType(feature, layer) {
                ...
                //const rtnum = feature.properties.RTNO;
        ...
        SC: {
            ...
            getFeatureRoadType(feature, layer) {
                const roadID = feature.properties.ROUTE_LRS;
                ...
                let fc = 7;
                switch (feature.properties[layer.fcPropName]) {
                    case 'INT': fc = 1; break;
                    case 'EXP': fc = 2; break;
                    case 'PRA': fc = 3; break;
                    case 'MIA': fc = 4; break;
        ...
        TN: {
            ...
            getFeatureRoadType(feature, layer) {
                ...
                let fc = STATE_SETTINGS.global.getRoadTypeFromFC(feature.properties.FUNC_CLASS, layer);
                if ((fc === 'PS' || fc === 'mH') && feature.properties.NBR_US_RTE != null) {
                    fc = feature.properties.NBR_US_RTE.endsWith('BR') ? 'mH' : 'MH';
                } else if (fc === 'PS' && (feature.properties.NBR_RTE.startsWith('SR') || feature.properties.NBR_RTE.startsWith('TN'))) {
                    fc = 'mH';
                }
                return fc;
            }
        ...
        TX: {
            ...
            getFeatureRoadType(feature, layer) {
                ...
                let fc = feature.properties[layer.fcPropName];
                const type = feature.properties.RTE_PRFX.substring(0, 2).toUpperCase();
                if (type === 'IH' && fc > 1) {
                    fc = 1;
        ...
        VT: {
            ...
            getFeatureRoadType(feature, layer) {
                const roadID = feature.properties.HWYSIGN;
                let fc = feature.properties[layer.fcPropName];
                if (!(fc > 0)) { fc = 7; }
        ...
        VA: {
            ...
            getFeatureRoadType(feature, layer) {
                ...
                let fc = parseInt(feature.properties[layer.fcPropName], 10);
                const rtName = feature.properties.RTE_NM;
                ...
                let rtNumText;
                if (layer.layerID === 1) {
                    rtNumText = feature.properties.ROUTE_NO;
                } else if (match) {
        ...
        WV: {
            ...
            getFeatureRoadType(feature, layer) {
                ...
                const fcCode = feature.properties[layer.fcPropName];
                let fc = fcCode;
                if (fcCode === 11) fc = 1;
    ...
    function getUrl(context, queryType, queryParams) {
        ...
        if (queryType === 'countOnly') {
            ...
            url += `&returnGeometry=true&maxAllowableOffset=${state.zoomSettings.maxOffset[zoom - 12] / 111000}`; // ${state.zoomSettings.maxOffset[zoom - 12] / 111000}Convert to degrees (4326) from the old Meters (3857)
        ...
        url += '&spatialRel=esriSpatialRelIntersects&geometryType=esriGeometryEnvelope&inSR=4326&outSR=4326&f=GEOJSON'; //&geometryPrecision=7  Not needed as it scales with maxAllowableOffset
    ...
    function convertFcToRoadTypeLineStrings(feature, context) {
      ...
      // Attributes to be associated with the feature
      const attr = {
        state: stateAbbr,
      ...
      // Create a LineString object manually using the GeoJSON input
      const lineString = {
        type: 'Feature',
        geometry: {
          type: 'LineString',
        ...
        id: feature.properties.OBJECTID, // or simply id: feature.id if that fits your use case
      };
      return lineString;
    }
    ...
    function fetchLayerFC(context) {
      ...
      if (!context.parentContext.cancel) {
        return getAsync(url, context).bind(context).then((res) => {
            try {
              const geoJson = JSON.parse(res.responseText);
              // First, try to access objectIds from properties, if properties exist
              let ids = [];
              if (geoJson.properties && geoJson.properties.objectIds) {
                ids = geoJson.properties.objectIds;
              } else if (geoJson.objectIds) {
                // Fallback to accessing objectIds from the root level if properties are not present
                ids = geoJson.objectIds;
              }
              if (ids.length === 0) {
              ...
              return {
                ids,
                objectIdFieldName: geoJson.properties ? geoJson.properties.objectIdFieldName : geoJson.objectIdFieldName,
              };
            } catch (err) {
          ...
          .then((result) => {
            const idRanges = [];
            const { ids, objectIdFieldName } = result;
            // Check if ids is an array and contains elements
            if (ids && ids.length) {
              ...
              while (currentIndex < len) {
                ...
                idRanges.push({
                  range: [ids[currentIndex], ids[nextIndex]],
                  idFieldName: objectIdFieldName,
                });
            ...
            if (!context.parentContext.cancel) {
              ...
              return getAsync(newUrl, context).then((res) => {
                if (!context.parentContext.cancel) {
                  ...
                  return features
                    .map((feature) => {
                      // Flatten MultiLineString into LineString if needed
                    ...
                    .filter((lineString) => lineString && lineString.properties && !(lineString.properties.roadType === 'St' && settings.hideStreet));
                }
                return null;
              });
            }
