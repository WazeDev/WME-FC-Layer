...
(function main() {
  ...
  function loadSettingsFromStorage() {
    ...
    Object.keys(defaultSettings)
      .filter((prop) => !_settings.hasOwnProperty(prop))
      .forEach((prop) => {
          ...
          if (!context.parentContext.cancel) {
            ...
            return getAsync(newUrl, context).then((res) => {
              if (!context.parentContext.cancel) {
                ...
                features = features || [];
                return features.map((feature) => convertFcToRoadTypeVectors(feature, context)).filter((vector) => !(vector[0].attributes.roadType === 'St' && _settings.hideStreet));
              }
              return null;
            });
          }
